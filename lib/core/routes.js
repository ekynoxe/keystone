...
function routes(app) {
	...
	var rolesMiddleware = require('../middleware/roles.js')(keystone);
	...
	if (keystone.get('cloudinary config')) {
		...
		app.get('/keystone/api/cloudinary/get', rolesMiddleware, require('../../routes/api/cloudinary').get);
		app.get('/keystone/api/cloudinary/autocomplete', rolesMiddleware, require('../../routes/api/cloudinary').autocomplete);
		app.post('/keystone/api/cloudinary/upload', rolesMiddleware, require('../../routes/api/cloudinary').upload);
	}

	// Generic Lists API
	debug('setting generic list api');
	app.all('/keystone/api/:list/:action(autocomplete|get|order|create|delete|fetch)', initList(), rolesMiddleware, require('../../routes/api/list'));
	app.get('/keystone/api/:list/:id', initList(), rolesMiddleware, require('../../admin/api/item/get'));

	debug('setting generic Lists download route');
	app.all('/keystone/download/:list', initList(), rolesMiddleware, require('../../routes/download/list'));

	// Admin-UI API
	debug('setting list and item details admin routes');
	app.all('/keystone/:list/:page([0-9]{1,5})?', initList(true), rolesMiddleware, require('../../routes/views/list'));
	app.all('/keystone/:list/:item', initList(true), rolesMiddleware, require('../../routes/views/item'));
